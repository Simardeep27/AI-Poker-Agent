...
RANGE_SIMS   = {'preflop': 36, 'flop': 30, 'turn': 26, 'river': 22}
BOARD_SAMPLES = 6
BASE_FOLD_RATE  = 0.35
PRIOR_STRENGTH  = 15
STREETS = {'preflop': 0, 'flop': 1, 'turn': 2, 'river': 3}
DEEPSTACK_BLEND = 0.42
...
def analyze_state(state):
    ...
    range_rate = _range_aware_win_rate(state, n_sims=RANGE_SIMS.get(street, 70))
    if range_rate is not None:
        line_weight = {
            'passive':      0.26,
            'single_raise': DEEPSTACK_BLEND,
            'reraise':      0.56,
        }.get(state['history_line'], DEEPSTACK_BLEND)
        if street == 'river':
            line_weight = min(0.70, line_weight + 0.12)
...
def choose_action(state, hand_info, response):
    ...
    if history_line == 'reraise':
        ...
        if hand_info['win_rate'] < 0.57 and raise_ok and raise_ev > -1e8:
            raise_ev -= 0.12 * pot
        if hand_info['win_rate'] < 0.42:
            call_ev -= 0.07 * pot
    ...
    if (raise_ok and hand_info['win_rate'] >= 0.62 and raise_ev > fold_ev
            and (response['fold'] > 0.22 or hand_info['win_rate'] >= 0.69)):
        return 'raise'
...
def _range_aware_win_rate(state, n_sims):
    ...
    for _ in range(n_sims):
        ...
        board_full_objs = gen_cards(board)
        hero_score = HandEvaluator.eval_hand(hero_objs, board_full_objs)
        opp_score = HandEvaluator.eval_hand(opp_objs, board_full_objs)
...
def _opponent_range_threshold(state):
    ...
    return max(0.12, min(0.72, threshold))
